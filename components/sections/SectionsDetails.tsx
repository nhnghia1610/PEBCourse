...
const SectionsDetails = ({
  ...
  const addToCart = async () => {
    try {
      ...
      console.log("Failed to add course to cart", err);
  ...
  return (
    <div className="px-6 py-4 flex flex-col gap-5">
      ...
      {isLocked ? (
        <div className="px-10 flex flex-col gap-5 items-center bg-gray-300">
          <Lock className="h-8 w-8" />
          <p className="text-sm font-bold">
            Nội dung bài học này chỉ dành cho học viên đã mua khóa học.
          </p>
        </div>
      ) : (
        <MuxPlayer
